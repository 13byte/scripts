...
class LogManager:
    """로그 관리를 위한 클래스"""
    def __init__(self, script_name: str, log_dir: str = "./script_logs", max_logs: int = 50):
        ...
        try:
            ...
            if len(log_files) > self.max_logs:
                for old_log in log_files[self.max_logs:]:
                    old_log.unlink()
                logging.info(f"Cleaned up {len(log_files) - self.max_logs} old log files")
...
def preview_changes(file_path: Path, old_text: str, new_text: str) -> Tuple[bool, List[Tuple[int, str, str]]]:
    """파일의 변경 사항을 미리 확인"""
    changes = []
    need_change = False
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f, 1):
                if old_text in line:
                    need_change = True
                    changes.append((i, line.strip(), line.replace(old_text, new_text).strip()))
...
def process_file(file_path: Path, old_text: str, new_text: str, preview_only: bool = False) -> Dict:
    """단일 파일 처리"""
    result = {
        'path': str(file_path),
        'modified': False,
